-- Keep append-only checkpoint history while allowing an operational wafer or die
-- to be removed from active process surfaces.
...
create index if not exists wafers_active_project_code_idx
  on public.wafers (project_id, wafer_code)
  where deleted_at is null;

create index if not exists wafer_process_assignments_active_template_idx
...
begin
  ...
  set
    status = 'scrapped',
    wafer_code = wafer.wafer_code || '__deleted__' || left(replace(wafer.id::text, '-', ''), 12),
