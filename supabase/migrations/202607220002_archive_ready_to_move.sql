-- Canonical archive eligibility includes checkpoint-approved "ready_to_move" steps.
-- This aligns client and DB eligibility checks after reviewer approvals.
    ...
    if not (
      ...
      or exists (
        ...
        where execution.assignment_id = assignment.id
          ...
          and execution.status in ('completed', 'ready_to_move')
      )
    ) then
