-- Fallback checkpoint-route movement when correction metadata is incomplete.
-- Keeps append-only history and still allows Beginning->destination movement for
-- active dice where the legacy metadata cannot be reconstructed.
...
begin
  ...
  if use_fallback then
    ...
    destination_status := case
      when destination_step.step_order <= checkpoint_step.step_order then 'redo_required'::public.step_status
