-- The live Process Flow treats lifecycle_status as compatibility metadata.
-- Active templates remain editable and may receive wafers regardless of that
-- historical value. Keep project authorization and atomic command semantics.
...
begin
  ...
  if not template.is_active then
    raise exception using errcode = '55000', message = 'Inactive processes cannot be edited.';
  end if;
...
begin
  ...
  if not template.is_active then
    raise exception using errcode = '55000', message = 'Only active processes can receive wafers.';
  end if;
