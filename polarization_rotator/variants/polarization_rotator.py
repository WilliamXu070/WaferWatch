...
def sidewall_rotator_layerbuilder(params, fdtd, only_update):
    """Inject the optimized rotator polygon into the sidewall Layer Builder."""
    fdtd.switchtolayout()

    points = np.asarray(polarization_rotator(params), dtype=float)
    if not np.allclose(points[0], points[-1]):
        points = np.vstack((points, points[0]))

    builder_x = _lumerical_scalar(fdtd.getnamed("WG_builder4", "x"))
    builder_y = _lumerical_scalar(fdtd.getnamed("WG_builder4", "y"))
    points = points - np.array([[builder_x, builder_y]])

    fdtd.putv("optimized_rotator_vertices", points)
    fdtd.eval('select("WG_builder4"); set("geometry", {"1:0":{optimized_rotator_vertices}});')
...
def runSim(initial_params, bounds, fsp_file, fsp_file2):
    ...
    depth=0.3e-6
    # Initialize sidewall-aware Layer Builder geometry.
    polygon = ParameterizedGeometry(func = sidewall_rotator_layerbuilder,
